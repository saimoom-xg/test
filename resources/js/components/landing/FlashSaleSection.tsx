...
export default function FlashSale({
    ...
    return (
        <div className="mb-20 glass-card rounded-[2rem] p-8 lg:p-10 border border-white relative overflow-hidden shadow-xl flex flex-col lg:flex-row items-center gap-12">
            ...
            <div className="w-full lg:w-1/2 relative z-10 text-center lg:text-left">
                ...
                <p className="text-brand-gray font-medium mb-10 max-w-sm mx-auto lg:mx-0">{description}</p>
                <div className="flex gap-4 justify-center lg:justify-start">
                    {[days, hours, mins, secs].map((num, i) => (
                        <div key={i} className="w-16 h-16 bg-white rounded-2xl flex flex-col items-center justify-center shadow-sm border border-gray-100">
                            ...
                            <span className="text-[10px] font-bold text-brand-gray uppercase">{['Days', 'Hours', 'Mins', 'Secs'][i]}</span>
                        </div>
                    ))}
                </div>
            </div>
            <div className="w-full lg:w-1/2 relative z-10 flex justify-center">
                <div className="relative">
                    <div className="absolute inset-0 bg-brand-yellow rounded-full blur-3xl opacity-30" />
                    <div className="relative z-10 w-80 drop-shadow-2xl -rotate-[15deg] hover:scale-110 transition-transform duration-500">
                        {productImage ? (
                            <img src={productImage} alt="Flash Sale" className="w-full h-auto object-contain mix-blend-multiply" />
                        ) : (
                    ...
                    <div className="absolute top-1/2 -right-4 bg-black text-white px-6 py-3 rounded-2xl shadow-xl rotate-12 z-20">
                        <p className="text-xs font-bold text-gray-400 line-through">{formatPrice(originalPrice)}</p>
                        <p className="text-xl font-bold">{formatPrice(salePrice)}</p>
                    </div>
                </div>
            </div>
        </div>
